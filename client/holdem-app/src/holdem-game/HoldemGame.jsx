...
export function HoldemGame() {
  ...
  const [gameSessionStatus, setGameSessionStatus] = useState(
    "Session uninitialised",
  );
  ...
  const handleError = (error) => {
    console.error(error);
    if (error.message === "SESSION_EXPIRED" || true) {
      // at the moment assume all errors caused by session expiration
      setGameSessionStatus(
        gameSessionId.length > 0 ? "Session expired" : "Session uninitialised",
      );
      setTimeout(() => {
        setGameSessionId("");
      }, 0);
  ...
  const tryCreateNewGame = () => {
    let timeoutID;
    console.log(
      "try creating new game, current gameSessionId =",
      gameSessionId,
    );
    if (gameSessionId?.length > 0) return;
    ...
    <>
      <div className="background">
        ...
        <div className="holdem-game">
          <Board
            cardStage={holdemState.cardStage}
            communityCards={communityCards}
          />
          <div className="center-flex" style={{ flexDirection: "column" }}>
            <button className="btn-shuffle" onClick={onShuffleBtnClick}>
              Shuffle
            </button>
            <div className="label-text" style={{ width: "fit-content" }}>
              Potsize: {potValue}
            </div>
          </div>
        </div>
        <Player
          ...
          id="0"
          enable={
            holdemState.cardStage !== "SHOW_DOWN" &&
            holdemState.playerStage === 0
          }
        ...
        <div
          ...
          hidden={holdemState.cardStage !== "SHOW_DOWN"}
        ...
        <div className="center-flex" style={{ flexDirection: "column" }}>
          <div
            ...
            hidden={true}
          >
            Player {holdemState.playerStage}'s turn
          </div>
          <button
            className="btn-next"
            hidden={holdemState.cardStage !== "SHOW_DOWN"}
